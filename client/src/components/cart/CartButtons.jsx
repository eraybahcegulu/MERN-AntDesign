...
const CartButtons = () => {
  ...
  const columns = [
    {
      title: <div className="text-center">Number</div>,
      dataIndex: 'number',
      render: (_, record, index) => (
        <div className="flex flex-row justify-center items-center">{index + 1}</div>
      ),
    },
    {
...

    {
      ...
      render: (_,record) => {
        ...
        return (
          <div className="flex flex-row justify-center items-center"><strong>{total}$</strong></div>
        );
      },
    },
  ];
        ...
        <div className="flex flex-col ">
          ...
          <div  className="flex flex-col mt-2">
            ...
            <div className="tax flex flex-row justify-between border-b mt-2">
              ...
              <span className="text-red-500">+{((cart.subTotal * cart.tax) / 100).toFixed(2)}$</span>
            </div>

            <div className="tax flex flex-row justify-between border-b mt-5">
              <span className="text-green-600 font-bold text-xl">TOTAL</span>
              <span className="text-green-600 text-xl">{(cart.subTotal + (cart.subTotal * cart.tax) / 100).toFixed(2)}$</span>
            </div>
